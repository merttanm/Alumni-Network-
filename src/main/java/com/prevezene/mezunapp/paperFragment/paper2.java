        ...
        call.enqueue(new Callback<List<paper_model>>() {
            ...
            public void onResponse(Call<List<paper_model>> call, Response<List<paper_model>> response) {
                if (response.isSuccessful()) {
                    List<paper_model> responseList2 = response.body();
                    papermodel = new ArrayList<>(responseList2);
                    //RecyclerView
                    recyclerView.setLayoutManager(new LinearLayoutManager(paper2.this));
                    recyclerViewAdapter2 = new RecyclerViewAdapter2(papermodel);
                    recyclerView.setAdapter(recyclerViewAdapter2);

                    for (paper_model Model : papermodel) {
                    //   System.out.println(Model.paper_one);
                        System.out.println(Model.paper_two);
