        ...
        call.enqueue(new Callback<List<paper_model>>() {
            ...
            public void onResponse(Call<List<paper_model>> call, Response<List<paper_model>> response) {
                if (response.isSuccessful()) {
                    List<paper_model> responseList4 = response.body();
                    papermodel = new ArrayList<>(responseList4);
                    //RecyclerView
                    recyclerView.setLayoutManager(new LinearLayoutManager(paper4.this));
                    recyclerViewAdapter4 = new RecyclerViewAdapter4(papermodel);
